'use strict'

const zmq = require('zeromq')
const exec = require('child_process').exec
const matrixMalos = require("matrix-protos").matrix_io.malos.v1
const driverConfig = matrixMalos.driver.DriverConfig
const everloopImage = matrixMalos.io.EverloopImage
const ledValue = matrixMalos.io.LedValue
let NUMBER_OF_LEDS = 35
const CREATOR_BOARD = '5c344e8'
const VOICE_BOARD = '6032bad2'
const PORT = 20013 + 8 //leds everloop port
const ERROR_PORT = 20013 + 8 + 2 //error everloop port

const MODE = {
  STATIC: 'static',
  GRADIENT: 'gradient',
  PULSE: 'pulse',
  RAINBOW: 'rainbow'
}

let LEDS = []

const RAINBOW = []

const DEFAULT_IDLE_MODE = {
  mode: MODE.RAINBOW,
  brightness: 0.2,
  leds: RAINBOW
}

module.exports = class MatrixLed {
  static get MODE() {
    return MODE
  }

  constructor(options) {
    if (typeof options === 'string') {
      options = {
        ip: options,
        idleMode: DEFAULT_IDLE_MODE
      }
    }
    if (options.idleMode) {
      options.idleMode = Object.assign(DEFAULT_IDLE_MODE, options.idleMode)
    } else {
      options.idleMode = DEFAULT_IDLE_MODE
    }

    const ip = options.ip
    this.idleMode = options.idleMode
    this.nbLeds = options.nbLeds || NUMBER_OF_LEDS
    this.stateMode = options.stateMode
    this.isIdle = true
    this.loopLed = 0
    this.animationSpeed = 60
    this.creatorIp = ip || '127.0.0.1'
    this._errorSocket = zmq.socket('sub')
    this._errorSocket.connect('tcp://' + this.creatorIp + ':' + ERROR_PORT)
    this._errorSocket.subscribe('')
    this._errorSocket.on('message', (error_message) => {
      const stringError = error_message.toString('utf8');
      if (stringError.indexOf('WARNING: Invalid number of Leds it must be') !== -1) {
        NUMBER_OF_LEDS = parseInt(stringError.split('=')[1]);
        for (let j = 0; j < NUMBER_OF_LEDS; j++) {
          const rgb = this._rainbow(j)
          RAINBOW[j] = rgb
        }
      }
      console.log('Message received: Pressure error: ' + error_message.toString('utf8') + '\n')
    })

    this._configSocket = zmq.socket('push')
    this._configSocket.connect('tcp://' + this.creatorIp + ':' + PORT)

    exec('/usr/share/matrixlabs/matrixio-devices/fpga_info', (error, stdout, stderr) => {
      if (stdout.indexOf(VOICE_BOARD) !== -1) {
        NUMBER_OF_LEDS = 18;
      }
      for (let j = 0; j < NUMBER_OF_LEDS; j++) {
        const rgb = this._rainbow(j)
        RAINBOW[j] = rgb
      }
      this.idle()
      this.start()
    });
  }

  _createEmptyMessage() {
    return driverConfig.create({ image: everloopImage.create() })
  }

  _HSVtoRGB(h, s, v) {
    let r, g, b, i, f, p, q, t;

    i = Math.floor(h * 6);
    f = h * 6 - i;
    p = v * (1 - s);
    q = v * (1 - f * s);
    t = v * (1 - (1 - f) * s);
    switch (i % 6) {
      case 0:
        r = v, g = t, b = p;
        break;
      case 1:
        r = q, g = v, b = p;
        break;
      case 2:
        r = p, g = v, b = t;
        break;
      case 3:
        r = p, g = q, b = v;
        break;
      case 4:
        r = t, g = p, b = v;
        break;
      case 5:
        r = v, g = p, b = q;
        break;
    }
    return {
      r: Math.round(r * 255),
      g: Math.round(g * 255),
      b: Math.round(b * 255)
    };
  }

  _rainbow(position) {
    return this._HSVtoRGB(position / parseFloat(this.nbLeds) * 0.85, 1.0, this.idleMode.brightness);
  }

  setColor(mode, rgb) {
    rgb = this._getRGBValue(rgb)
    this.stop()
    const message = this._createEmptyMessage()
    if (mode !== MODE.PULSE) {
      for (let j = 0; j < this.nbLeds; ++j) {
        if (mode === MODE.STATIC) {
          message.image.led.push(this._getLedValue(rgb))
        }
        else if (mode === MODE.GRADIENT) {
          const redDelta = Math.floor(rgb.r / this.nbLeds) * 1.1
          const greenDelta = Math.floor(rgb.g / this.nbLeds) * 1.1
          const blueDelta = Math.floor(rgb.b / this.nbLeds) * 1.1
          const whiteDelta = Math.floor(rgb.w / this.nbLeds) * 1.1

          LEDS[j] = {
            r: rgb.r - (j * redDelta),
            g: rgb.g - (j * greenDelta),
            b: rgb.b - (j * blueDelta),
            w: rgb.w - (j * whiteDelta)
          }
        }
      }
    }
    if (mode === MODE.STATIC) {
      this._send(message)
    }
    else if (mode === MODE.GRADIENT) {
      this.setColors(mode, LEDS)
    }
    else if (mode === MODE.PULSE) {
      this._setPulse(rgb, Math.max(rgb.r, rgb.g, rgb.b, rgb.w))
    }
  }

  _setPulse(rgb, maxIntensity) {
    this._stopIdle()
    this._stopState()
    const message = this._createEmptyMessage()
    for (let j = 0; j < this.nbLeds; ++j) {
      message.image.led.push(this._getLedValue(this._getRGBValue({
          r: rgb.r - this.loopLed,
          g: rgb.g - this.loopLed,
          b: rgb.b - this.loopLed,
          w: rgb.w - this.loopLed
        }))
      )
    }
    this._send(message)
    this.stateTimer = setInterval(() => {
      this.loopLed += Math.ceil(maxIntensity / this.nbLeds)
      if (this.loopLed >= maxIntensity) {
        this.loopLed = 0
      }
      this._setPulse(rgb, maxIntensity)
    }, 10)
  }

  setColors(mode, rgbs) {
    if (rgbs.length !== this.nbLeds) {
      throw new Error(`need ${this.nbLeds} values`)
    }
    this._stopIdle()
    const message = this._createEmptyMessage()
    for (let j = this.loopLed; j < this.loopLed + this.nbLeds; ++j) {
      let index = j
      if (index > this.nbLeds - 1) {
        index -= this.nbLeds - 1
      }
      message.image.led.push(this._getLedValue(this._getRGBValue(rgbs[index])))
    }
    this._send(message)
    this._stopState()

    if (mode === MODE.GRADIENT) {
      this.stateTimer = setInterval(() => {
        this.loopLed += 1
        if (this.loopLed >= this.nbLeds) {
          this.loopLed = 0
        }
        this.setColors(mode, rgbs)
      }, this.animationSpeed)
    }
  }

  idle() {
    this._stopState()
    if (this.isIdle) {
      const message = this._createEmptyMessage()
      for (let j = this.loopLed; j < this.loopLed + this.nbLeds; ++j) {
        let index = j
        if (index > this.nbLeds - 1) {
          index -= this.nbLeds - 1
        }
        const rgb = this.idleMode.leds[index]
        message.image.led.push(this._getLedValue(rgb))
      }
      this._send(message)
    }
    else {
      this.start()
    }
  }

  start() {
    this.isIdle = true
    if (this.loopLed > this.nbLeds) {
      this.loopLed = 0
    }
    this.idle()
    if (this.idleTimer) clearInterval(this.idleTimer)

    this.idleTimer = setInterval(() => {
      this.loopLed += 1
      if (this.loopLed >= this.nbLeds) {
        this.loopLed = 0
      }
      this.idle()
    }, this.animationSpeed)
  }

  _stopState() {
    if (this.stateTimer) clearInterval(this.stateTimer)
    this.stateTimer = null
  }

  _stopIdle() {
    this.isIdle = false
    if (this.idleTimer) clearInterval(this.idleTimer)
    this.idleTimer = null
  }

  stop() {
    this._stopIdle()
    this._stopState()
  }

  quit() {
    this.stop()
    this.setColor('static', { r: 0, g: 0, b: 0 })
  }

  _send(message) {
    this._configSocket.send(driverConfig.encode(message).finish());
  }

  _getRGBValue(rgb) {
    return {
      r: Math.max(rgb.r || 0, 0),
      g: Math.max(rgb.g || 0, 0),
      b: Math.max(rgb.b || 0, 0),
      w: Math.max(rgb.w || 0, 0)
    }
  }

  _getLedValue(rgb) {
    return ledValue.create({
      red: rgb.r,
      green: rgb.g,
      blue: rgb.b,
      white: rgb.w
    })
  }
}
